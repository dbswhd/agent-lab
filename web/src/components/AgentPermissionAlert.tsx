...
export function AgentPermissionAlert({
  ...
  useEffect(() => {
    ...
    setClaudeAgentLab(Boolean(d.claude?.local_agent_lab  ?? FULL_AGENT_PERMISSIONS.claude.local_agent_lab));
    setClaudePipeline(Boolean(d.claude?.local_pipeline   ?? FULL_AGENT_PERMISSIONS.claude.local_pipeline));
    setClaudeLectureScript(Boolean(d.claude?.local_lecture_script ?? FULL_AGENT_PERMISSIONS.claude.local_lecture_script));
  }, [open]);
  ...
  function handleConfirm() {
    const perms = buildPermissionsFromForm(selectedAgents, {
      cursorTools, cursorAgentLab, cursorPipeline, cursorLectureScript,
      codexCli,
      claudeTools, claudeWrite, claudeAgentLab, claudePipeline, claudeLectureScript,
    });
      ...
      <div className="perm-form">
        {showCursor && (
          <fieldset className="perm-group">
            ...
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={cursorTools}
                onChange={(e) => setCursorTools(e.target.checked)} />
              도구 사용 (파일 읽기·검색)
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={cursorAgentLab}
                onChange={(e) => setCursorAgentLab(e.target.checked)} />
              agent-lab 프로젝트 접근
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={cursorPipeline}
                onChange={(e) => setCursorPipeline(e.target.checked)} />
              quant-pipeline 접근
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={cursorLectureScript}
        ...
        {showCodex && (
          <fieldset className="perm-group">
            <legend>Codex</legend>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={codexCli}
        ...
        {showClaude && (
          <fieldset className="perm-group">
            ...
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={claudeTools}
                onChange={(e) => setClaudeTools(e.target.checked)} />
              도구 사용 (Claude Code — 읽기·검색)
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={claudeWrite}
                onChange={(e) => setClaudeWrite(e.target.checked)} />
              파일 편집 (Claude Code acceptEdits)
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={claudeAgentLab}
                onChange={(e) => setClaudeAgentLab(e.target.checked)} />
              agent-lab 프로젝트 접근
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={claudePipeline}
                onChange={(e) => setClaudePipeline(e.target.checked)} />
              quant-pipeline 접근
            </label>
            <label className="perm-check">
              <input type="checkbox" className="checkbox" checked={claudeLectureScript}
