#!/usr/bin/env python3
"""Mypy error-count ratchet for ``agent_lab.workspace`` (strict overrides in pyproject.toml)."""
...
BASELINE_PATH = ROOT / "tests" / "fixtures" / "mypy-workspace-ratchet.json"
PKG_SRC = ROOT / "src" / "agent_lab" / "workspace"
...
def main() -> int:
    ...
    if args.print:
        print(f"workspace total={total} ratchet={ratchet} max_allowed={max_allowed}")
    ...
    if args.update:
        payload = {
            ...
            "note": "Strict overrides: pyproject.toml [[tool.mypy.overrides]] agent_lab.workspace.*",
        }
    ...
    if ratchet > max_allowed:
        print(f"workspace mypy ratchet FAILED: {ratchet} errors > baseline {max_allowed}", file=sys.stderr)
        return 1

    print(f"workspace mypy ratchet OK: {ratchet}/{max_allowed} (total={total})")
    return 0
