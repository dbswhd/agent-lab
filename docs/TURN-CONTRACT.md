...
> **Code SSOT:** `src/agent_lab/room/turn_contract.py` · `turn_contract_feedback.py` · `turn_policy.py` · `turn_flow_phases.py`
...
`observe_turn()`은 topic과 현재 run state에서 다음 증거를 만든다.
...
`99 agents`는 새 roster를 만드는 값이 아니라 현재 roster를 자르지 않는 상한이다.
...
`task_kind + risk + execute_intent`가 맞는 outcome이 10건 이상이면 후보별 평균 결과를 bounded adjustment로 반영한다. legacy outcome에서 context가 비어 있으면 호환 표본으로 취급한다.
...
| `adaptive` | source가 `history` 또는 `explore`일 때만 적용 |
