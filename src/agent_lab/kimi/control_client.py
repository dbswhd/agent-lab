...
def _refresh_bridge_on_probe_failure() -> None:
    ...
    from agent_lab.kimi.daimon_supervisor import is_owned_pid, read_lock_owner_pid, shutdown_owned_daimon
...
def _resolve_live_endpoint() -> ControlEndpoint:
    ...
    from agent_lab.kimi.daimon_supervisor import ensure_daimon
    ...
    for attempt in range(_BRIDGE_RETRY_ATTEMPTS):
        try:
            ...
            if is_transient_bridge_error(exc) and attempt + 1 < _BRIDGE_RETRY_ATTEMPTS:
                invalidate_endpoint_cache()
                from agent_lab.kimi.daimon_supervisor import shutdown_owned_daimon
