#!/usr/bin/env python3
"""One-shot: move context_*.py → context/ and rewrite agent_lab.context_* imports."""
...
IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.context_(\w+)"), r"agent_lab.context.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("context_*.py")):
        target_stem = path.stem[len("context_") :]
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/context/")
    for old, new in moved:
        print(f"  {old} -> context/{new}")
