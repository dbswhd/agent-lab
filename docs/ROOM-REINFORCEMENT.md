...
> **Phase I (shipped):** Execute worktree M0–M4 — worktree dry-run/merge, isolation override, orphan guard, score KPI (`EXECUTE-WORKTREE-REFORM.md` §11). Live Cursor SDK dry-run은 수동 Go/No-Go만 유지.
...
CI: `pytest tests/ -q` + `scripts/smoke_room.py` (14 baselines) + `check_worktree_orphans.py` + `score_session.py --json` fixture smoke (LLM/secrets 없음).
...
주간 리포트용 KPI. H-P1에서 regression fixture JSON smoke가 CI에 연결됨.
...
1. **H-P4** — `mailbox_handoff`, `specialist_asymmetric_cwd` regression → `smoke_room.py` SCENARIOS (bridge/objection 패턴)
2. **실사용 KPI** — `score_session` 주간 리포트 + M4 마일스톤 수치 (objection 해결률 >80%, execute 재시도율 <30%)
3. **Live execute** — `CURSOR_API_KEY` 환경에서 worktree dry-run 1회 재검증 (문서화만, CI secrets 없음)
