...
> **Stale notice (2026-06):** Part 5 Phase 3 ops items, **CENT-env** (subprocess allowlist), and the mock-first Layer 3 execute verify/agent repair loop are **shipped**. Layer 4 adversarial gate is **🔶 mock skeleton** only. See **[EXTERNAL-REFS-TRACEABILITY.md](EXTERNAL-REFS-TRACEABILITY.md)** for plan ↔ code ↔ fixture mapping.
...
없는 것 (또는 mock만):
  ...
  adversarial gate — live Claude + UI wiring (mock skeleton만 🔶 — adversarial_gate.py)
...
Layer 4: Adversarial Gate (🔶 mock skeleton — LC-L4)
  dry-run diff 생성 → Claude에게 "이게 실패할 수 있는 이유?" 1회 질문
  결과를 Human approve UI에 표시 (차단은 안 함, 정보 제공)
  mock: adversarial_gate.py + sessions/_regression/adversarial_gate_lgtm/
...
| Layer 4: Adversarial Gate | 🔶 mock skeleton | `adversarial_gate.py`, `sessions/_regression/adversarial_gate_lgtm/` — §1.5 |
| Layer 5: Goal-Driven Loop | ❌ 미구현 (선택적) | — |
| Durable Step | ✅ shipped | `run_meta.py`, `room.py`, `sessions/_regression/durable_completed_steps/` — Part 3 |
...
|---|------|-----------|------|--------------|----------|
| 1 | Socratic interview (session_clarifier 강화) | 🔶 feature-flag off | §1.3 | LC-clarifier | P1 |
| 2 | Adversarial Gate (diff 반증 시도) | 🔶 mock skeleton | §1.5 | LC-L4 | P1 |
| 3 | Oracle Verified Completion (독립 검증자) | ❌ 미구현 | §1.6 | LC-oracle | P1 |
| 4 | AGENTS.md / PROJECT.md 영속 메모리 | ❌ 미구현 | §1.7 | MD-PROJECT | P2 |
