"""Typed run.json state — F11 Stage 1 boundary (ADR §3.5)."""
...
def validate_run_data(run: Mapping[str, Any]) -> None:
    """Runtime invariants for run.json — absorbed from ``run/schema.py``."""
...
class RunState(dict[str, Any]):
    ...
    Subclasses ``dict`` so existing ``isinstance(x, dict)`` guards and duck-typed
    call sites keep working while ``read_run_meta()`` establishes a typed boundary.
    """

    @classmethod
    def from_raw(cls, raw: Mapping[str, Any] | None) -> RunState:
        data = dict(raw or {})
        validate_run_data(data)
        return cls(data)

    @classmethod
    def from_memory(cls, raw: Mapping[str, Any] | None) -> RunState:
        """In-memory turn payload — no validation (ephemeral keys, partial updates)."""
...
RunStateLike = RunState | dict[str, Any]
