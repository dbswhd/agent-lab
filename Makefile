...
test-integration: check-worktrees
	.venv/bin/python scripts/run_verification_lane.py --lane integration --marker-expression "integration and not quant and not live and not bridge" -- .venv/bin/pytest tests/ -q -m "integration and not quant and not live and not bridge"

test-quant: check-worktrees
	@.venv/bin/python -c 'from agent_lab.extensions.quant_trading import optional_agentic_src; src = optional_agentic_src(); print(f"quant compatibility: {src}" if src else "quant compatibility: extension not installed (tests will skip)")'
	.venv/bin/python -m pytest tests/ -q -ra -m "quant and not live"
...
pipeline-dogfood-run:
	AGENT_LAB_MOCK_AGENTS=1 AGENT_LAB_MISSION_LOOP=1 .venv/bin/python scripts/pipeline_dogfood_run.py
...
dogfood-readiness-report:
	@test -n "$(MANIFEST)" || (echo "Usage: make dogfood-readiness-report MANIFEST=path/to/manifest.json [OUT_DIR=path] [CHECK=1]" && exit 1)
	.venv/bin/python scripts/dogfood_readiness_report.py \
	  --manifest "$(MANIFEST)" \
	  --out-dir "$(if $(OUT_DIR),$(OUT_DIR),.omo/evidence/dogfood-readiness)" \
	  $(if $(CHECK),--check,)
