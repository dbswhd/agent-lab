...
11. `.agent-lab/self_patch_allowlist.txt` — N6
...
| ID | 작업 | 파일 |
|----|------|------|
| HS3-1 | `harness_proposer.py` | 신규 |
| HS3-2 | manifest.json SSOT; allowlist 단일화 | `.agent-lab/harness/` |
| HS3-3 | prompts BLOCK 파서 | `harness_proposer.py` |
| HS3-4 | trigger: recurrence≥3, addressable, STOP OK | `weakness_miner.py` |
| HS3-5 | proposer: offline script 우선; Room optional | `scripts/propose_harness.py` |
| HS3-6 | PatchCandidate가 신규 glob/블록/플래그 도입 시 대응 dogfood topic 또는 evals case를 함께 제안 (REVIEW P0-5, F2 플래그-프로필 가드와 동형) | `harness_proposer.py` |

Self-Harness bounded context 4항목 (R11) 필수. eval 표면 동시 확장 검사는 HS4-6이 거부 게이트로 집행 — HS3-6은 제안 측 의무.

플래그: `AGENT_LAB_HARNESS_PROPOSER` (default `0`)
...
1. Proposer: Room vs `scripts/propose_harness.py` only (권장: **offline 우선**)  
