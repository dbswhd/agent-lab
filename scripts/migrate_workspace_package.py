#!/usr/bin/env python3
"""One-shot: move workspace_*.py → workspace/ and rewrite agent_lab.workspace_* imports."""
...
PKG_DIR = SRC / "workspace"

IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.workspace_(\w+)"), r"agent_lab.workspace.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("workspace_*.py")):
        target_stem = path.stem[len("workspace_") :]
        target = PKG_DIR / f"{target_stem}.py"
    ...
    if not init.is_file():
        init.write_text(
            '"""Workspace roots, files API, and per-workspace markdown guidance."""\n\nfrom __future__ import annotations\n',
            encoding="utf-8",
        )
    return moved
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/workspace/")
    for old, new in moved:
        print(f"  {old} -> workspace/{new}")
