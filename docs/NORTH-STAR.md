...
| **N6** | **Self-patch meta-loop** | Room이 생성한 개선안을 agent-lab 자기 코드베이스에 적용하는 dogfood 루프의 정례화. `self_patch.py`(화이트리스트 classify, 관측만 — **코어 로직은 여전히 인간 gate 필수**) + HSIL 2단계 설계 [DESIGN-HARNESS-SELF-IMPROVE.md](./DESIGN-HARNESS-SELF-IMPROVE.md) **APPROVED**(Liner 연구 6종 검토 [REVIEW-LINER-RESEARCH-2026-07.md](./REVIEW-LINER-RESEARCH-2026-07.md) 기반 P0 5건 반영). **HS0(ATTRIB)~HS5(MERGE, HS5-3 포함) 코드+mock 검증 완료** — mine→propose→regress→merge 파이프라인 E2E 코드 완결. **다음:** HS6(DGM-lite)은 동결 — HS-M5 게이트(Human을 통한 실제 self-patch 1건)가 실사용 데이터 부재로 미충족(2026-07-09 검토, NOW.md §1). 이력: §3.3.1 3차 wave | L3 | D2 |
