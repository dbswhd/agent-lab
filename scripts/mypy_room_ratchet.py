#!/usr/bin/env python3
"""Mypy error-count ratchet for ``agent_lab.room`` (strict overrides in pyproject.toml).

Usage:
    python scripts/mypy_room_ratchet.py --check
    python scripts/mypy_room_ratchet.py --update
...
BASELINE_PATH = ROOT / "tests" / "fixtures" / "mypy-room-ratchet.json"
ROOM_SRC = ROOT / "src" / "agent_lab" / "room"
...
def main() -> int:
    ...
    parser.add_argument("--check", action="store_true", help="Exit 1 if room error count exceeds baseline")
    parser.add_argument("--update", action="store_true", help="Rewrite room ratchet baseline")
    parser.add_argument("--print", action="store_true", help="Print current room counts vs baseline")
    args = parser.parse_args()
    if not (args.check or args.update or args.print):
        parser.error("one of --check, --update, or --print is required")

    _code, counts = run_room_mypy()
    ...
    if args.print:
        print(f"room total={total} ratchet={ratchet} max_allowed={max_allowed}")
    ...
    if args.update:
        payload = {
            ...
            "note": "Strict overrides: pyproject.toml [[tool.mypy.overrides]] agent_lab.room.*",
        }
    ...
    if ratchet > max_allowed:
        print(
            f"room mypy ratchet FAILED: {ratchet} errors > baseline {max_allowed}",
            file=sys.stderr,
        )
        print(
            f"  total={total} — fix types or run: python scripts/mypy_room_ratchet.py --update",
            file=sys.stderr,
        )
        return 1

    print(f"room mypy ratchet OK: {ratchet}/{max_allowed} (total={total})")
    return 0
