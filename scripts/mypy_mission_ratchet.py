#!/usr/bin/env python3
"""Mypy error-count ratchet for ``agent_lab.mission``."""
...
BASELINE_PATH = ROOT / "tests" / "fixtures" / "mypy-mission-ratchet.json"
PKG_SRC = ROOT / "src" / "agent_lab" / "mission"
...
def main() -> int:
    ...
    if args.print:
        print(f"mission total={total} ratchet={ratchet} max_allowed={max_allowed}")
        return 0
    if args.update:
        payload = {
            "version": 1,
            ...
            "note": "Strict overrides: pyproject.toml [[tool.mypy.overrides]] agent_lab.mission.*",
        }
    ...
    if ratchet > max_allowed:
        print(f"mission mypy ratchet FAILED: {ratchet} errors > baseline {max_allowed}", file=sys.stderr)
        return 1
    print(f"mission mypy ratchet OK: {ratchet}/{max_allowed} (total={total})")
    return 0
