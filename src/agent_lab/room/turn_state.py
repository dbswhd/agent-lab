...
def derive_turn_state(
    ...
    *,
    ...
    if status == "reached" and anchor_dict:
        ...
        who = agent_label(str(anchor_dict.get("agent", "")))
...
def render_turn_state_block(state: dict[str, Any] | TurnState | None) -> str:
    ...
    if ts.anchor:
        a = ts.anchor
        who = agent_label(str(a.get("agent", "")))
    ...
    if ts.pending_agents:
        names = ", ".join(agent_label(a) for a in ts.pending_agents)
...
def current_turn_slice(
    ...
    for i, m in enumerate(all_messages):
        if m.role == "user":
            last_user = i
    if last_user < 0:
        return list(all_messages), 1
    return list(all_messages[last_user:]), last_user + 1
