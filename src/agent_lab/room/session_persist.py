...
def _append_human_turn_synthesis(
    ...
    *,
    ...
    if not should_emit_human_turn_synthesis(
        turn_profile, turn_slice, agents_used=list(agents_used) if agents_used else None
    ):
        return messages
...
def _resolve_discuss_objections_from_consensus(
    ...
    *,
    ...
    if not isinstance(consensus, dict):
        return
    from agent_lab.room.objections import resolve_objections_on_endorse
...
def _write_session_files(
    ...
    *,
    ...
    sync_session_meta(
        run_meta,
        topic=topic,
        messages=cast(Any, messages_to_store),
        plan_md=plan_md,
    ...
    from agent_lab.room.mailbox import harvest_mailbox_from_turn
    ...
    from agent_lab.room.hooks import _hook_run_record, run_post_harvest_hooks
    ...
    if plan_changed and plan_md.strip():
        from agent_lab.plan.provenance import extract_plan_provenance

        from agent_lab.room.tasks import RUN_PLAN_PROVENANCE_KEY
