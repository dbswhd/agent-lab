#!/usr/bin/env python3
"""One-shot: move quant_*.py → quant/ and rewrite agent_lab.quant_* imports."""
...
IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.quant_(\w+)"), r"agent_lab.quant.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("quant_*.py")):
        target_stem = path.stem[len("quant_") :]
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/quant/")
    for old, new in moved:
        print(f"  {old} -> quant/{new}")
