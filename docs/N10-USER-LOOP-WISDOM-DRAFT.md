...
| S3a-0 인벤토리 | **D2** (2026-07-06 구현) | ✅ `plugin_discovery.py` 결과를 도구 카드로 래핑(`tool_cards.py`) + 카테고리 태깅(결정론적 키워드, LLM 미사용) + `feedback_advisor.advise_setup()`(RECALL) rationale에 미사용 카드 제안 부착 + `SetupHint.tool_card_suggestions` 구조화 필드 → `turns[].category`→`turn_metrics`→outcome ledger로 관통 → `feedback_report.py`에 `tool_cards.tool_card_hit_rate` (23 mock 테스트: `test_tool_cards.py` 10 + advisor/report 확장 13) | `tool_card_hit_rate` > 0 실세션 관측 | **완료(MVP)** — `src/agent_lab/tool_cards.py`. **스코프 축소:** "미사용"은 세션이 아직 명시적으로 enable하지 않은 도구(`agent_plugins` raw, `merge_session_allowlist`의 default-enabled 백필 제외)로 정의 — 과거 세션에 걸친 실제 호출 이력 추적은 아님(그런 원장이 존재하지 않음). `tool_card_hit_rate`도 "제안 시점에 clean-pass한 비율"의 근사치이며 채택(adoption) 여부까지는 추적하지 않음 |
| N10b Rule Sync | **D2** (2026-07-06 구현) | ✅ `.agent-lab/wisdom/correction_rules.md` 파싱 → `.claude/rules/correction-rules.md`·`.cursor/rules/correction-rules.mdc`·`~/.codex/AGENTS.md` 각각 delimited-section 렌더 → dry-run(`preview_rule_sync`) + Human Inbox(`rule_sync` kind, N10a promote 직후 제안) 승인 시에만 실제 쓰기(`apply_rule_sync`) — 재동기화해도 마커 밖 손글씨 내용 보존 확인 (`tests/test_rule_sync.py` 14 cases + `test_correction_harvester.py` 2 cases). **실브라우저 검증 중 사고 기록(2026-07-06):** Codex 타깃 검증 시 `AGENT_LAB_CODEX_HOME` env override가 미리보기 실행기에 전달되지 않아 실제 사용자 전역 `~/.codex/AGENTS.md`에 테스트 내용이 1회 기록됐다가 즉시 원상복구(0바이트)함 — 코드 자체의 결함은 아니었음(override가 실제로 전달됐을 때는 유닛 테스트로 정확히 격리 확인됨)이나, 향후 유사 검증 시 API 프로세스에 안전한 override가 실제로 전달됐는지 먼저 확인 필요 | Human 승인 flow 포함 실사용 1회(안전하게 재확인 필요) | **완료** — `src/agent_lab/rule_sync.py`, 플래그 `AGENT_LAB_RULE_SYNC`(**default 0** — 유일하게 외부 전역 파일에 쓰는 N10 계열 기능이라 다른 4개와 달리 default OFF), `AGENT_LAB_CODEX_HOME`(override, infra) |
