#!/usr/bin/env python3
"""One-shot: move wisdom_*.py → wisdom/ and rewrite agent_lab.wisdom_* imports."""
...
IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.wisdom_(\w+)"), r"agent_lab.wisdom.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("wisdom_*.py")):
        target_stem = path.stem[len("wisdom_") :]
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/wisdom/")
    for old, new in moved:
        print(f"  {old} -> wisdom/{new}")
