#!/usr/bin/env python3
"""Fail when code imports legacy ``agent_lab.plan_*`` paths.

Canonical imports are ``agent_lab.plan.<module>``.

Usage:
    python scripts/audit_plan_legacy_imports.py
    python scripts/audit_plan_legacy_imports.py --json
"""
...
def collect_violations() -> list[dict[str, str | int]]:
    ...
    for base in SCAN_ROOTS:
        ...
        for path in sorted(base.rglob("*.py")):
            if path.name == "migrate_plan_package.py":
                continue
            for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                if "plan_*" in line and "agent_lab" not in line:
                    continue
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"Legacy plan_* imports: {len(hits)}")
        for row in hits:
            print(f"  {row['path']}:{row['line']}: {row['text']}")
    else:
        print("Legacy plan_* imports: 0 (OK)")
