...
from agent_lab.plan.execute_paths import normalize_snapshot_path
...
def _read_snapshot_bytes(folder: Path, exec_id: str, entry: dict[str, Any]) -> bytes | None:
    ...
    if path.is_file():
        return cast(bytes, path.read_bytes())
    return None
...
def restore_snapshot(
    ...
    *,
    ...
    for rel, file_entry in files_meta.items():
        norm = normalize_path(rel, cwd=cwd)
        target = _resolve_under(cwd, norm)
        if file_entry.get("existed"):
            snap_file = file_entry.get("snapshot_file")
            if not snap_file:
                continue
