#!/usr/bin/env python3
"""Mypy error-count ratchet for ``agent_lab.agent`` (not ``agent_lab.agents``)."""
...
BASELINE_PATH = ROOT / "tests" / "fixtures" / "mypy-agent-ratchet.json"
PKG_SRC = ROOT / "src" / "agent_lab" / "agent"
...
def main() -> int:
    ...
    if args.print:
        print(f"agent total={total} ratchet={ratchet} max_allowed={max_allowed}")
        return 0
    if args.update:
        payload = {
            "version": 1,
            ...
            "note": "Strict overrides: pyproject.toml [[tool.mypy.overrides]] agent_lab.agent.*",
        }
    ...
    if ratchet > max_allowed:
        print(f"agent mypy ratchet FAILED: {ratchet} errors > baseline {max_allowed}", file=sys.stderr)
        return 1
    print(f"agent mypy ratchet OK: {ratchet}/{max_allowed} (total={total})")
    return 0
