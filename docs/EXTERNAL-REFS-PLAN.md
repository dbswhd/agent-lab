...
> **Stale notice (2026-06):** Part 5 Phase 3 ops items and **CENT-env** (subprocess allowlist) are **shipped**. Layer 4 adversarial gate is **🔶 mock skeleton** only. Layer 3 execute verify loop remains **⬜**. See **[EXTERNAL-REFS-TRACEABILITY.md](EXTERNAL-REFS-TRACEABILITY.md)** for plan ↔ code ↔ fixture mapping.
...
| Layer 3: Execute Verify Loop | ❌ 미구현 | `plan_execute_merge.py` — §1.4 |
...
| Durable Step | ❌ 미구현 | `run_meta.py` + `room.py` — Part 3 |
