#!/usr/bin/env python3
"""One-shot: move inbox_*.py → inbox/ and rewrite agent_lab.inbox_* imports."""
...
IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.inbox_(\w+)"), r"agent_lab.inbox.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("inbox_*.py")):
        target_stem = path.stem[len("inbox_") :]
    ...
    if not init.is_file():
        init.write_text(
            '"""Human inbox harvest, facilitator, and MCP servers."""\n\nfrom __future__ import annotations\n',
            encoding="utf-8",
        )
    return moved
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/inbox/")
    for old, new in moved:
        print(f"  {old} -> inbox/{new}")
