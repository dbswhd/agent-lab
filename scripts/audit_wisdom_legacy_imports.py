#!/usr/bin/env python3
"""Fail when code imports legacy agent_lab.wisdom_* root paths."""
...
LEGACY_RE = re.compile(r"\b(?:from|import)\s+agent_lab\.wisdom_[a-z_]+|\bagent_lab\.wisdom_[a-z_]+\b")
    ...
    for base in SCAN_ROOTS:
        ...
        for path in sorted(base.rglob("*.py")):
            ...
            for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                if "wisdom_*" in line and "agent_lab" not in line:
                    continue
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"Legacy wisdom_* imports: {len(hits)}")
        for row in hits:
            print(f"  {row['path']}:{row['line']}: {row['text']}")
    else:
        print("Legacy wisdom_* imports: 0 (OK)")
