#!/usr/bin/env python3
"""Fail when code imports legacy ``agent_lab.agent_*`` paths (not ``agents`` registry)."""
...
LEGACY_RE = re.compile(r"\b(?:from|import)\s+agent_lab\.agent_[a-z_]+|\bagent_lab\.agent_[a-z_]+\b")
    ...
    for base in SCAN_ROOTS:
        ...
        for path in sorted(base.rglob("*.py")):
            ...
            for lineno, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                if "agent_*" in line and "agent_lab" not in line:
                    continue
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"Legacy agent_* imports: {len(hits)}")
        for row in hits:
            print(f"  {row['path']}:{row['line']}: {row['text']}")
    else:
        print("Legacy agent_* imports: 0 (OK)")
