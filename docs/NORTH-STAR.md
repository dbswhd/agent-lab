...
| **N6** | **Self-patch meta-loop** | Room이 생성한 개선안을 agent-lab 자기 코드베이스에 적용하는 dogfood 루프의 정례화. **첫 커밋 ✅ (2026-07-07, D1):** `src/agent_lab/self_patch.py` + committed `.agent-lab/self_patch_allowlist.txt`(초기값: `.claude/skills/**`·`agents/prompts.py`·`run/profile.py`) — gitignore-style `**`/`*` 매칭, `classify_self_patch()`는 순수 함수(부작용 없음). `outcome_harvester.record_execute_outcome`에 `self_patch` 필드로 관측만 부착(게이트 변경 없음 — 화이트리스트 안이든 밖이든 오늘과 동일하게 전부 Human Inbox 경유), `feedback_report.py`에 `self_patch_eligible_rate` 집계 추가(24 mock 테스트). **코어 로직(`src/agent_lab/**`)은 여전히 인간 gate 필수** — 새로 강제한 게 아니라 화이트리스트에 없다는 사실 자체로 구조적으로 보장됨. **다음(미착수):** 실제 자기수정 루프(Room이 개선안을 생성 → 화이트리스트 안에서 자동/경량 승인으로 적용)는 별도 후속 작업 | L3 | 1단계 완료 · 루프 자체는 1달~분기 |
