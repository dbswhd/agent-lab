...
| **N10** | **User-Loop Wisdom** | 사용자 턴을 S1 입력 차원으로 편입 ([N10-USER-LOOP-WISDOM-DRAFT.md](./N10-USER-LOOP-WISDOM-DRAFT.md)). N10a Correction Harvester(`correction_harvester.py`, `AGENT_LAB_CORRECTION_HARVESTER` default 1) — 반복 교정 패턴을 Human Inbox 승인 후 규칙으로 영속. N10b Rule Sync(`rule_sync.py`, `AGENT_LAB_RULE_SYNC` default 0, Human 승인 필수) — SSOT를 하네스별(Claude/Cursor/Codex) 포맷으로 단방향 export. 이력: §3.3.1 3차 wave | L1 | D2 |
