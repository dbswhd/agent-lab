#!/usr/bin/env python3
"""Analyze plan package import graph (mirrors room_import_graph.py).

Usage:
    python scripts/plan_import_graph.py
    python scripts/plan_import_graph.py --json
"""
...
def _plan_modules() -> dict[str, Path]:
...
def _parse_imports(path: Path) -> tuple[list[tuple[str, tuple[str, ...]]], list[tuple[str, tuple[str, ...]]]]:
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
            if not node.module or not node.module.startswith("agent_lab.plan"):
                return
...
def collect_graph() -> dict:
    ...
    plan_module_names = set(modules)
    for path in SRC.rglob("*.py"):
        if path in modules.values():
            continue
        mod = _module_name(path)
        if mod.startswith("plan"):
            continue
        tree = ast.parse(path.read_text(encoding="utf-8"))
    ...
    in_degree = Counter({name: 0 for name in plan_module_names})
    out_degree = Counter({name: 0 for name in plan_module_names})
    for edge in edges:
        if edge.source in plan_module_names and edge.target in plan_module_names:
    ...
    return {
        "plan_module_count": len(modules) - (1 if FACADE in modules else 0),
...
def _print_human(payload: dict) -> None:
    print(f"plan package import graph ({payload['plan_module_count']} modules + package root)")
