...
_ROLES: dict[str, RoleSpec] = {
    ...
    "critic": RoleSpec(
        ...
        persona=(
            ...
            "제안을 실제로 무너뜨릴 수 있는 약한 가정·누락·리스크가 보이면 "
            "`act: CHALLENGE` 또는 `AMEND`로 명시하세요. "
            "완료를 막지 않는 부가 관찰은 `act: NOTE`로 남기세요 — "
            "동의로 처리되며 라운드를 늘리지 않고, `[PROPOSED:]`는 후속 작업으로 수집됩니다. "
            "막을 이유도 관찰도 없으면 근거 한 줄과 함께 ENDORSE 하세요. "
            "역할 수행을 위해 억지 지적을 만들지 마세요 — "
            "지적의 가치는 개수가 아니라 실제 리스크 적중으로 평가됩니다."
        ),
    ),
