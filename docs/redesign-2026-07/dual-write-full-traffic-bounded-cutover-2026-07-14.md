...
> **Current runtime note (M6-9):** The soak approval and enable card below are historical evidence. The dual-write bridge now requires a non-empty session allowlist; an empty or missing allowlist fails closed. Slice 1–3 authority flags/functions are retired, ignored, and fail-closed, so no authority enable command in historical records is executable.

## 승인 단계 (의도된 순서)

```text
1. Controlled cohort evidence     ✅ GO (v3d) — cohort run report
2. Human: Full traffic 승인       ✅ 2026-07-14 (≥15 Room turns)
3. Full traffic 운영 데이터 수집  ✅ soak PASS (15/15, hard_mm=0, dup=0)
...
| Flag | 전용 API process `DUAL_WRITE=1` + **non-empty allowlist** | shared process + **non-empty allowlist** |
| Legacy writer | 유지 | **유지** |
...
| 항목 | 기본값 | 의미 |
| --- | --- | --- |
| Process 격리 | **전용 uvicorn** (`AGENT_LAB_MISSION_DUAL_WRITE=1` + non-empty `AGENT_LAB_MISSION_DUAL_WRITE_SESSIONS`) | dogfood/UI traffic만 이 포트로 |
...
```bash
# 전용 process (non-empty allowlist required)
...
- Process: 전용 uvicorn :8765 + non-empty allowlist (또는 shared + non-empty allowlist)
...
Soak ledger: `/tmp/agent-lab-dw-full-traffic-20260714/` · script: `scripts/mission_dual_write_full_traffic_soak.py`
결과: **PASS** — 15/15 turns · verify hard_mm=0 · journal duplicate=0 (2026-07-14)
```
