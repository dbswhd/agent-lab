...
describe("concept panel quality gate", () => {
  it("allows an explicit plan request only for a ready selection", () => {
    ...
    expect(
      canRequestPlan({ ...base, selection: { option_id: "opt-review" } }),
    ).toBe(false);
  ...
  it("does not label a failed or pending plan as ready", () => {
    expect(
      stageView({ ...base, stage: "plan", plan_status: "failed" })?.label,
    ).toBe("계획 생성 실패");
    expect(
      stageView({ ...base, stage: "plan", plan_status: "pending" })?.label,
    ).toBe("계획 준비 중");
    expect(
      stageView({ ...base, stage: "plan", plan_status: "ready" })?.label,
    ).toBe("계획 준비됨");
  });
});
