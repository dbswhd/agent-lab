...
def _peer_metrics_for_messages(messages: list[ChatMessage]) -> dict[str, Any]:
    from agent_lab.room.turn_state import current_turn_slice, peer_turn_metrics
...
def _final_turn_state_dict(
    ...
    *,
    ...
    if run_meta and run_meta.get("turn_state"):
        return cast(dict[str, Any], run_meta["turn_state"])
    from agent_lab.room.turn_state import current_turn_slice, derive_turn_state
...
def synthesize_session_plan(
    ...
    *,
    ...
    try:
        ...
        def _mark_plan_ready(run: dict[str, Any]) -> dict[str, Any]:
        ...
        def _mark_plan_failed(run: dict[str, Any]) -> dict[str, Any]:
            state = dict(run.get("ideation") or {})
            if state:
...
def _try_dispatch_turn(
    *,
    ...
    from agent_lab.room.dispatch import try_dispatch_turn
