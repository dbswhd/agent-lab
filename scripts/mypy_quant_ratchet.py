#!/usr/bin/env python3
"""Mypy error-count ratchet for ``agent_lab.quant`` (strict overrides in pyproject.toml)."""
...
BASELINE_PATH = ROOT / "tests" / "fixtures" / "mypy-quant-ratchet.json"
PKG_SRC = ROOT / "src" / "agent_lab" / "quant"
...
def main() -> int:
    ...
    if args.print:
        print(f"quant total={total} ratchet={ratchet} max_allowed={max_allowed}")
    ...
    if args.update:
        payload = {
            ...
            "note": "Strict overrides: pyproject.toml [[tool.mypy.overrides]] agent_lab.quant.*",
        }
    ...
    if ratchet > max_allowed:
        print(f"quant mypy ratchet FAILED: {ratchet} errors > baseline {max_allowed}", file=sys.stderr)
        return 1

    print(f"quant mypy ratchet OK: {ratchet}/{max_allowed} (total={total})")
    return 0
