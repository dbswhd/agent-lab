#!/usr/bin/env python3
"""One-shot: move run_*.py → run/ and rewrite agent_lab.run_* imports."""
...
IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.run_(\w+)"), r"agent_lab.run.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("run_*.py")):
        target_stem = path.stem[len("run_") :]
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/run/")
