#!/usr/bin/env python3
"""Manual M0 Go/No-Go: live Cursor SDK dry-run in an isolated git worktree."""
...
from scripts.soak.live_execute_spike import format_report_lines, run_live_worktree_spike
...
def main() -> int:
    ...
    _load_env()
    report = run_live_worktree_spike(
...
_USAGE = """Usage: live_cursor_worktree_dry_run.py (requires AGENT_LAB_RUN_LIVE=1)

Runs one real Cursor dry-run in a disposable git worktree. Main repo tree stays
clean; execution is rejected after verify so no merge is required.
