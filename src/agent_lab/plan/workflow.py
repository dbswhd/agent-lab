...
def _clarifier_topic(run: dict[str, Any]) -> str:
...
def _inbox_question_surfaces_prompt(run: dict[str, Any], prompt: str) -> bool:
    from agent_lab.human_inbox import inbox_items
...
def ensure_plan_clarify_interview(folder: Path) -> dict[str, Any] | None:
    ...
    if clarity_threshold_met(run):
        return None
    hold = _clarity_gate_questions(folder, run)
...
def ensure_plan_clarify_inbox_question(folder: Path) -> dict[str, Any] | None:
    ...
    pending = _pending_clarifier_questions(interview)
    if not pending:
        return None
        ...
        for item in inbox_items(run):
            if item.get("kind") == "question" and item.get("status") == "pending":
                if str(item.get("prompt") or "").strip() == prompt:
                    return item
        return None
    if _inbox_question_surfaces_prompt(run, prompt):
        for item in inbox_items(run):
            if item.get("kind") == "question" and str(item.get("prompt") or "").strip() == prompt:
                return item
        return None
    topic = _clarifier_topic(run)
    question = pending[0]
...
def build_plan_clarify_agent_block(folder: Path, *, agent_id: str, run_meta: dict[str, Any] | None) -> str:
    ...
    interview = get_clarifier_interview(run)
    pending_q = _pending_clarifier_questions(interview)
    if not pending_q:
        return ""
    topic = _clarifier_topic(run)
    q = pending_q[0]
