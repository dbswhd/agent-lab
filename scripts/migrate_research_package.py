#!/usr/bin/env python3
"""One-shot: move research_*.py → research/ and rewrite agent_lab.research_* imports."""
...
PKG_DIR = SRC / "research"

IMPORT_REWRITES = ((re.compile(r"\bagent_lab\.research_(\w+)"), r"agent_lab.research.\1"),)
...
def move_modules() -> list[tuple[str, str]]:
    ...
    for path in sorted(SRC.glob("research_*.py")):
        target_stem = path.stem[len("research_") :]
    ...
    if not init.is_file():
        init.write_text(
            '"""Research MCP server, read helpers, and artifact cards."""\n\nfrom __future__ import annotations\n',
            encoding="utf-8",
        )
    return moved
...
def main() -> None:
    moved = move_modules()
    print(f"Moved {len(moved)} modules into src/agent_lab/research/")
    for old, new in moved:
        print(f"  {old} -> research/{new}")
