...
Agent Lab hook **세 겹 + worktree lifecycle** — Layer A는 **에이전트별·이벤트별** 라우팅 shipped (2026-06-07).

> **정정 (2026-07-10):** Worktree hooks(`.agent-lab/worktree.yaml`)는 Layer C(native product hooks)가 **아니다**. execute lane lifecycle(create/setup/verify/remove)이며 **3-layer와 별도**. 정확한 표현 = **3-layer + worktree lifecycle**.
> **ABS-P2-hooks:** CC `SubagentStart`/`SubagentStop`을 `pre_agent_reply`/`post_agent_reply`/`task_completed`에 **runtime alias 금지** — lifecycle·fail 정책이 다름 (`room/hooks.py` `_SHIPPED_EVENT_POLICIES` / `_TARGET_EVENT_POLICIES`). 지금은 문서 유사점 매핑만.

```
┌─────────────────────────────────────────────────────────────┐
│ Layer A — Room hooks (orchestrator) ✅ shipped               │
│   src/agent_lab/room/hooks.py + hooks.toml                   │
│   events: pre_execute | task_completed | teammate_idle       │
│         + pre_agent_reply | post_agent_reply | post_harvest  │
│         + pre_scribe | pre_dispatch | post_dispatch          │
│   scope: per-agent [hooks.cursor|codex|claude] + global      │
│   정책: event별 fail-closed / feedback-only / retry 분리     │
├─────────────────────────────────────────────────────────────┤
...
└─────────────────────────────────────────────────────────────┘
         + (별도) Execute worktree lifecycle — MB-6 / ABS-P2-worktree-yaml
             .agent-lab/worktree.yaml · worktree_hooks.py
             create → setup → verify → remove  (≠ Layer C)
```
...
**모듈:** `src/agent_lab/room/hooks.py`
...
**주의:** `src/agent_lab/room/hooks.py`(Room runtime)와 **별개** (`CLAUDE.md`, `EXTERNAL-REFS-TRACEABILITY` CC-hooks).
...
|------|----------------|-------------|
| Room hooks | `src/agent_lab/room/hooks.py` | `test_room_hooks`, `test_hook_router` |
...
| 날짜 | 내용 |
|------|------|
| 2026-07-10 | 경로 drift 정정 `room_hooks.py` → `room/hooks.py`; **3-layer + worktree lifecycle** 명시; ABS-P2-hooks = 문서 매핑만 (SubagentStart/Stop runtime alias 금지) |
