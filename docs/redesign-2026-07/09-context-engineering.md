...
**2026-07-16 — 실제 세션 코호트로 표본 확대(`scripts/context_recipe_shadow_real_session_cohort.py`, `make context-recipe-shadow-real-cohort`).** 지금까지 전부 손으로 만든 synthetic run_meta였는데, 이번엔 (1) 저장소의 real regression fixture 7개(`sessions/_regression/` — `mission_loop_discuss_recovery`/`wisdom_index_built`/`mission_loop_plan_reject`/`mission_loop_execute_queue`/`plan_workflow_pw5_latency`/`evidence_gates_merged_ok`/`evidence_ledger_stream`, 전부 진짜 `run.json`의 `mission_loop`/`topic`/`executions` 상태, `plan_md`/`messages`/`workspace_binding`/artifact 1건만 보충 — fixture가 구조적 smoke-test 베이스라인이라 그것들은 원래 없음)와 (2) `mission_dogfood_run.py`와 동일한 실제 `mission/loop.py`/`mission/advance.py`/`verified_loop.py` 함수로 새로 구동한 세션 1건(Oracle verdict를 `fail`로 줘서 REPAIR phase 도달 — `run.json`/`chat.jsonl`/`plan.md` 전부 진짜, artifact만 보충)을 기반으로 8개 시나리오를 돌렸다.
