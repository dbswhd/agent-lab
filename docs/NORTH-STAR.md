...
| 회귀 베이스라인 | `make test-fast` (mock 전량 — 정확한 수는 실행 출력 참조, `tests/test_integration_registry.py`가 상한 ratchet) · `python scripts/smoke_room.py` (베이스라인 수는 `sessions/_regression/` 참조) · `make dogfood-suite-mock` |
...
| `harness_failure_rate` / `model_resolved_rate` | execute-phase outcome 중 harness 귀속 비율 / harness 실패 제외 model 해결률 (HS0 ATTRIB) | `feedback_report` · `harness_attribution` (`eval_harness.aggregate()`) | **now** (HS0 shipped 2026-07-08) |
...
| **N2** | **프로필 시스템** | 전체 플래그(`make list-flags`) → `fast`/`small`/`balanced`/`thorough`/`autonomous` 프로필 매핑 (`run/profile.py`). `small` = Small Mission (supervisor + Oracle live + lean efficiency; moat-safe). 개별 override 유지. 신규 feature 플래그는 프로필 소속 권장 (`make list-flags --profile`). **위험역전 방지 핀**(N10 개정, 축소 스코프): `risk_pin.py` — trading 카테고리 감지 시 autonomy ceiling **L1** 고정, `AGENT_LAB_RISK_PIN` default 1. 이력·근거: §3.3.1 3차 wave | L1 | D2 |
...
| **N6** | **Self-patch meta-loop — 철회 (2026-08-14)** | **철회:** 실사용 1,429 verdict-eligible 행에서 `self_patch_eligible_rate = 0.0`, `harness_patch = null`. HS1~HS5 모듈 6개(1,690 LOC)와 플래그 4종 제거 — 어느 프로필에서도 켜진 적 없어 동작 변화 없음. HS0 ATTRIB(`eval_harness`)만 유지. 재개하려면 실사용 표본으로 HS1 태깅 > 0을 먼저 증명해야 한다. 이하 원문 이력: | Room이 생성한 개선안을 agent-lab 자기 코드베이스에 적용하는 dogfood 루프의 정례화. `self_patch.py`(화이트리스트 classify, 관측만 — **코어 로직은 여전히 인간 gate 필수**) + HSIL 2단계 설계 [DESIGN-HARNESS-SELF-IMPROVE.md](./DESIGN-HARNESS-SELF-IMPROVE.md) **APPROVED**(Liner 연구 6종 검토 [REVIEW-LINER-RESEARCH-2026-07.md](./archive/legacy/REVIEW-LINER-RESEARCH-2026-07.md) 기반 P0 5건 반영). **HS0(ATTRIB)~HS5(MERGE, HS5-3 포함) 코드+mock 검증 완료** — mine→propose→regress→merge 파이프라인 E2E 코드 완결. **다음:** HS6(DGM-lite)은 동결 — HS-M5 게이트(Human을 통한 실제 self-patch 1건)가 실사용 데이터 부재로 미충족(2026-07-09 검토, NOW.md §1). 이력: §3.3.1 3차 wave | L3 | D2 |
| **N7** | **S3 외부 능력 자가 통합** | §1 Layer 1의 S3a~S3d. **설계 문서 ✅ (2026-07-07):** [`S3-TOOL-CARD-SPEC.md`](./S3-TOOL-CARD-SPEC.md) — 도구 카드 스키마 확장(S3a-0 5개 필드 불변 + source/capabilities/mount 옵셔널 필드) · `[NEED-TOOL:]` 시그널 문법(`[LEARNED:]`/`[PROPOSED:]`와 동일 컨벤션) · Human Inbox `tool_mount` kind 승인 flow · S3c outcome 관통(`mounted_hit_rate`) · S3d 자기적용 판정 기준. **구현은 S1/S2 닫힌 후** (부품: 코드 앵커 맵 S3 행) | L3 | 설계 완료 · 구현 분기 |
...
| **N10** | **User-Loop Wisdom** | 사용자 턴을 S1 입력 차원으로 편입 ([N10-USER-LOOP-WISDOM-DRAFT.md](./N10-USER-LOOP-WISDOM-DRAFT.md)). N10a Correction Harvester(`correction_harvester.py`, `AGENT_LAB_CORRECTION_HARVESTER` default 1) — 반복 교정 패턴을 Human Inbox 승인 후 규칙으로 영속. ~~N10b Rule Sync~~ **철회 (2026-08-14)** — `correction_patterns.total = 1`, `AGENT_LAB_RULE_SYNC`는 한 번도 켜지지 않음. 이력: §3.3.1 3차 wave | L1 | D2 |
...
**3차 wave (2026-07-06~08 shipped):**

| ID | 산출 |
|----|------|
| **N2 risk_pin** (2026-07-06, D2, 축소 스코프) | `risk_pin.py` — `topic_router`의 `trading` 카테고리를 턴 종료 시점에 감지하면 autonomy ceiling **L1** 고정(기존 N4 demotion 경로 재사용, 신규 Inbox kind 없음); 완화는 "Restore ceiling" 명시 override만, 세션당 카테고리 1회. 프로필 하한(`thorough`)은 미구현(`apply_run_profile`이 `os.environ` 전역 변경이라 세션 단위 하향 불가). `AGENT_LAB_RISK_PIN` default 1. 근거: 사용 진단 G4(위험이 클수록 검증이 느슨해지는 역전) |
| **N10a Correction Harvester** (2026-07-06, D2) | `correction_harvester.py` — 사용자 교정 발화를 `outcomes.jsonl`의 `user_correction` episode로 수확 → `MIN_SAMPLE`(3) 이상 distinct 세션 관측 시 Human Inbox `correction_rule` 카드 → approve 시 `.agent-lab/wisdom/correction_rules.md` 영속. `AGENT_LAB_CORRECTION_HARVESTER` default 1. `feedback_report.py`에 `correction_patterns`/`correction_recurrence_rate` 집계, 19 mock 테스트 |
| **N10b Rule Sync** (2026-07-06, D2) | `rule_sync.py` — SSOT를 하네스별 포맷(Claude `.claude/rules/` · Cursor `.cursor/rules/` · Codex `AGENTS.md`)으로 delimited-section 단방향 export. Human 승인 필수(신규 Inbox kind `rule_sync`) — 승인 전 dry-run만, 마커 밖 기존 내용 항상 보존. `AGENT_LAB_RULE_SYNC` **default 0**(N10 계열 중 유일하게 외부 전역 파일에 쓰므로 명시 opt-in) |
| **N6 self_patch 첫 커밋** (2026-07-07, D2) | `self_patch.py` + committed `.agent-lab/self_patch_allowlist.txt`(초기값 `.claude/skills/**`·`agents/prompts.py`·`run/profile.py`, gitignore-style 매칭) — `classify_self_patch()` 순수 함수. `outcome_harvester.record_execute_outcome`에 `self_patch` 필드 관측만 부착(게이트 변경 없음, 전부 여전히 Human Inbox 경유). `feedback_report.py`에 `self_patch_eligible_rate` 집계, 24 mock 테스트. 코어 로직(`src/agent_lab/**`)은 화이트리스트 밖이라는 사실 자체로 인간 gate 구조적 보장 |
| **N6 HSIL 설계 APPROVED** (2026-07-08) | [DESIGN-HARNESS-SELF-IMPROVE.md](./DESIGN-HARNESS-SELF-IMPROVE.md) — ATTRIB→MINE→PLAYBOOK→PROPOSE→REGRESS→MERGE 6단계 로드맵(HS0~HS7) 확정. Liner 연구 6종 검토([REVIEW-LINER-RESEARCH-2026-07.md](./archive/legacy/REVIEW-LINER-RESEARCH-2026-07.md)) 기반 P0 5건(분류기-검증기 게이트 원칙 · 해결 패턴 누적 셋 · 롤백/메모리 드리프트 격리 · hooks.toml Tier B 재분류 · 평가 표면 동시 확장) 반영 |
| **N6 HS0 ATTRIB** (2026-07-08, D2) | `eval_harness.py`에 `score_dogfood_status`/`score_outcome_verdict` 어댑터 + `harness_failure_rate`, `feedback_report.py`·`run_dogfood_suite.py`에 `harness_attribution` 관측 wiring. `AGENT_LAB_EVAL_HARNESS` default ON. §1.4 KPI 편입 |
| **N6 HS1 MINE** (2026-07-08, D2) | `turn_metrics.py`에 `failure_tags`(harness_infra·false_success·weak_taste) + `weakness_miner.py`(신규 — 턴별 trace, memory_store 첫 consumer, 패턴 재발 마이닝). `AGENT_LAB_WEAKNESS_MINER` default 0 |
| **N6 HS2 PLAYBOOK** (2026-07-08, D2) | `wisdom/playbook.py`(신규 — append-only curator, 동일 pattern_id는 evidence_count만 증가) + `correction_harvester.py` dual-write + `context/bundle.py` R1 주입. `AGENT_LAB_PLAYBOOK` default 0 |
| **N6 HS3 PROPOSE** (2026-07-09, D2) | `harness_proposer.py`(신규) — `.agent-lab/harness/manifest.json` SSOT(Tier A/B/C·axis·frozen prefix, 기존 `self_patch_allowlist.txt` 대체·R4 해소), STOP guard·Tier·axis·eval-surface 4단 게이트, `prompts.py` BLOCK 마커 파서. Offline CLI `scripts/propose_harness.py`("offline script 우선, Room optional" — HS3-5). `AGENT_LAB_HARNESS_PROPOSER` default 0(`autonomous.owns`) |
...
- **완성도 ⚠:** Layer 3 = **60% (D2)** (§3.1 슈퍼 샘플 준비도와 동일 수치로 통일 — 2026-07-04 감사 시점 45%에서 §3.1 갱신 반영). QUICKSTART·FORK·mock 재현 ✅. **잔여:** clean-clone `fork_time` 자동화 · live emergence · T2.
