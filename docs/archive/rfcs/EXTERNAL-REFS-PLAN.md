...
없는 것 (선택·티켓 없음):
  (없음 — Layer 5 LC-L5 shipped; 추가 확장은 제품 로드맵)
```
...
```
Phase 0 — ✅ CENT-env, CC-CLAUDE, CC-hooks
Phase 1 — ✅ LC-L3/L4 (mock-first + live opt-in), LC-oracle, CENT-durable
Phase 2 — ✅ MD-PROJECT/PLATFORM/P3, CON-diff, LC-clarifier, CC-rules/skills
Phase 3 — ✅ ops-P0/P2, H-P1, H-P2, smoke 20 baselines
Phase 4 — ✅ LC-L5 goal loop, PI-executed archive

신규 external-ref 코드 티켓: 없음 (TRACEABILITY §Next implementation candidates)
```
...
| **P1** | LazyCodex | Adversarial Gate (Layer 4) | S | ✅ LC-L4 — mock + live opt-in |
| **P1** | LazyCodex | Execute Verify Loop (Layer 3) | M | ✅ LC-L3 |
| **P1** | LazyCodex | Oracle Verified Completion (§1.6) | M | ✅ LC-oracle |
| **P1** | Centaur | Durable Step (completed_steps) | S | ✅ CENT-durable |
| **P2** | LazyCodex | PROJECT.md 영속 메모리 (§1.7) | S | ✅ MD-PROJECT |
| **P2** | Conductor | Diff viewer 인라인 재작업 | L | ✅ CON-diff |
| **P2** | LazyCodex | session_clarifier 강화 | S | ✅ LC-clarifier |
| **P2** | Claude Code | Subagent skills | XS | ✅ CC-skills |
| **P2** | LazyCodex | Goal-driven session loop (Layer 5) | M | ✅ LC-L5 |
| **P2** | Conductor | PI-executed diff archive | S | ✅ PI-executed |
...
Agent Lab (shipped):
  ┌─ 토론 합의 + objection gate (Agent Lab 고유)
  ├─ 격리 실행 + CON-diff revise (Conductor, Phase I)
  ├─ Verify + repair loop (LC-L3) + adversarial note (LC-L4)
  ├─ Goal loop + mock Oracle (LC-L5)
  ├─ Durable Step (CENT-durable) + PI-executed archive
  └─ Provenance + Human-gated execute
```
