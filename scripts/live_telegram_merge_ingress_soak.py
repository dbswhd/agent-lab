#!/usr/bin/env python3
"""Live soak: Telegram webhook `/approve merge` ingress (Tier D)."""

from __future__ import annotations

import json
import os
import sys
from pathlib import Path

_ROOT = Path(__file__).resolve().parents[1]
if str(_ROOT / "src") not in sys.path:
    sys.path.insert(0, str(_ROOT / "src"))
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from dotenv import load_dotenv

from agent_lab.app_config import apply_config_env


def _load_env() -> None:
    apply_config_env()
    home_env = Path.home() / ".agent-lab" / ".env"
    if home_env.is_file():
        load_dotenv(home_env, override=False)
    repo_env = _ROOT / ".env"
    if repo_env.is_file():
        load_dotenv(repo_env, override=False)


def main() -> int:
    argv = sys.argv[1:]
    as_json = "--json" in argv
    keep = "--keep-artifacts" in argv
    write_path: Path | None = None
    work_parent: Path | None = None

    if "-h" in argv or "--help" in argv:
        print(_USAGE, file=sys.stderr)
        return 0

    if os.getenv("AGENT_LAB_RUN_LIVE", "").strip() not in {"1", "true", "yes"}:
        print(
            "Refusing to run without AGENT_LAB_RUN_LIVE=1 (safety guard).",
            file=sys.stderr,
        )
        print(_USAGE, file=sys.stderr)
        return 1

    i = 0
    while i < len(argv):
        arg = argv[i]
        if arg in ("--json", "--keep-artifacts"):
            i += 1
            continue
        if arg == "--write" and i + 1 < len(argv):
            write_path = Path(argv[i + 1]).expanduser()
            i += 2
            continue
        if arg == "--work-dir" and i + 1 < len(argv):
            work_parent = Path(argv[i + 1]).expanduser()
            i += 2
            continue
        print(f"Unknown argument: {arg}", file=sys.stderr)
        return 1

    _load_env()
    from scripts.soak.live_telegram_merge_soak import (
        format_telegram_soak_lines,
        run_live_telegram_merge_ingress_soak,
    )

    report = run_live_telegram_merge_ingress_soak(
        work_parent=work_parent,
        cleanup=not keep and work_parent is None,
    )

    if write_path:
        write_path.parent.mkdir(parents=True, exist_ok=True)
        write_path.write_text(
            json.dumps(report, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )

    if as_json:
        print(json.dumps(report, indent=2, ensure_ascii=False))
    else:
        for line in format_telegram_soak_lines(report):
            print(line)

    status = report.get("status")
    if status == "go":
        return 0
    if status == "skipped":
        return 3
    return 2


_USAGE = """Usage: live_telegram_merge_ingress_soak.py (requires AGENT_LAB_RUN_LIVE=1)

Runs one real Cursor dry-run in a disposable git worktree, then approves merge via
POST /api/gateway/telegram/webhook with `/approve merge` (not pytest mocks).

Environment:
  CURSOR_API_KEY          Required for dry-run worktree
  AGENT_LAB_SKIP_LIVE=1   Skip without calling Cursor

Options:
  --json              Machine-readable report on stdout
  --write PATH        Save report JSON (no secrets)
  --work-dir PATH     Reuse parent dir (repo/ + session/ + config/)
  --keep-artifacts    Do not delete temp work dir

Exit codes: 0=go, 2=no_go, 3=skipped, 1=usage/guard
"""


if __name__ == "__main__":
    raise SystemExit(main())
