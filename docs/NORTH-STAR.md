...
| **N2** | **프로필 시스템** | 전체 플래그(`make list-flags`) → `fast`/`balanced`/`thorough`/`autonomous` 4개 프로필 매핑 (`run/profile.py`). 개별 override 유지. 신규 feature 플래그는 프로필 소속 권장 (`make list-flags --profile`). **위험역전 방지 핀**(N10 개정, 축소 스코프): `risk_pin.py` — trading 카테고리 감지 시 autonomy ceiling **L1** 고정, `AGENT_LAB_RISK_PIN` default 1. 이력·근거: §3.3.1 3차 wave | L1 | D2 |
